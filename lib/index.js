    ...
    if (req.query.action === 'config') {
      ...
      busboy.on('file', function (fieldname, file, filename, encoding, mimetype) {
        ...
        res.ue_up = function (img_url, cb) {
          /**
           * 如果是上传到七牛，就有config.qn
           * 如果是上传到阿里云，就有config.ali
           * 七牛文档: https://developer.qiniu.com/kodo/sdk/1289/nodejs
           * 阿里云文档: https://help.aliyun.com/document_detail/32068.html?spm=a2c4g.11186623.6.1196.28864c60auo2Bs
           *
           */
          if (config.qn) {
            ...
            var name = uuid() + path.extname(filename)
            ...
            formUploader.putStream(uploadToken, key, file, PutExtra, function (err, results) {
              ...
              if (!results) return cb(new Error('results为空，七牛服务出错'))
            ...
            // 阿里云oss流式上传 https://help.aliyun.com/document_detail/111267.html?spm=a2c4g.11186623.6.1205.1392165b9VAZfu
            let client = new alioss(config.ali);

            client.putStream(key, file).then(results => {
              cb(null, {
                'mimetype': mimetype,
                'key': results.name,
                'url': config.ali.host + '/' + results.name,
                // 'url': results.url,
                'original': filename,
                'state': 'SUCCESS'
              })
            ...
            var name = uuid() + path.extname(tmpdir)

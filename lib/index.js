        ...
        if (req.query.action === 'config') {
            ...
            if (config.qn) {
                res.ue_list = function () {
                    let qn_client = qn.create(config.qn);

                    let dates = [];
                    for(var i = 0; i < 30; i ++){
                        dates.push(`/${config.qn.prefix}/${moment().add(-i,'days').format('YYYY-MM-DD')}/`); // /ue/20170908/
                    }
                    qn_client.list(dates.join(','), function (err, result) {
                        let list = ((result || {}).items || []).sort((a,b) => {
                                return b.putTime - a.putTime;
                    }).map(item => {
                            return {
                                "url": config.qn.origin + '/' + item.key,
                                "original": '',
                                "state": 'SUCCESS'
                            }
                        })
            ...
            busboy.on('file', function (fieldname, file, filename, encoding, mimetype) {
                ...
                res.ue_up = function (img_url) {
                    ...
                    if (config.qn) {
                        ...
                        client.upload(file, {
                            key: `${config.qn.prefix}/${moment().format('YYYY-MM-DD')}/${name}`			//例如： ue/20170908/dafhfjaadfkjsdhjdf.jpg
                            ...
                            res.end(JSON.stringify(resp));		//todo: 单图上传有问题, 嵌套的iframe获取不到内容
                        });
                        return false
                    }
                    if (config.fdfs) {
